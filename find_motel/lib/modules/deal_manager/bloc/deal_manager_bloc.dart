...
class DealManagerBloc
    extends Bloc<DealManagerEvent, DealManagerState> {
  ...
  Future<void> _onLoadDeals(
    ...
    final (deals, error) = await _customerService.fetchDeals(saleId: userProfile?.email ?? '');
    if (error != null) {
      emit(state.copyWith(
      ...
      emit(state.copyWith(
        status: DealManagerStatus.success,
        deals: deals ?? [],
      ));
    }
  }
